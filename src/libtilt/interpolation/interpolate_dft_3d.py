...
def sample_dft_3d(
    ...
    dft = einops.rearrange(torch.view_as_real(
        dft), 'd h w complex -> complex d h w')
    dft = einops.repeat(dft, 'complex d h w -> b complex d h w', b=n_samples)
    coordinates = einops.rearrange(
        coordinates, 'b zyx -> b 1 1 1 zyx')  # b d h w zyx
...
def insert_into_dft_3d(
    ...
    if data.shape != coordinates.shape[:-1]:
        raise ValueError(
            'One coordinate triplet is required for each value in data.')
    ...
    with torch.device(device):
        ...
        in_volume_idx = (coordinates >= 0) & (
            coordinates <= torch.tensor(dft.shape) - 1)
        ...
        def add_data_at_corner(z: Literal[0, 1], y: Literal[0, 1], x: Literal[0, 1]):
            ...
            zc, yc, xc = einops.rearrange(corners[:, [z, y, x], [0, 1, 2]],
                                          'b zyx -> zyx b')
            dft.index_put_(indices=(zc, yc, xc),
                           values=w * data, accumulate=True)
